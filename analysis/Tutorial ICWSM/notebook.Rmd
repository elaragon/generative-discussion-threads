...
plotdegrees <- ggplot(df.degrees_all, aes(x=degree, y=cumprob, color=data)) + 
  geom_point(aes(x=degree, y = cumprob), shape = 21, size = 1, stroke = 1) +
  geom_line(aes(x=degree, y = cumprob), size=0.25) +
  scale_y_log10(breaks=c(seq(0.1,1,0.1))) +
...
plotsubtrees <- ggplot(df.subtrees_all, aes(x=size, y=cumprob, color=data)) + 
  geom_point(aes(x=size, y = cumprob), shape = 21, size = 1, stroke = 1) +
  geom_line(aes(x=size, y = cumprob), size=0.25) +
  scale_y_log10(breaks=c(seq(0.1,1,0.1))) +
  scale_x_log10(breaks=c(1,10,100)) +
  theme_bw() +
  theme(panel.grid.major = element_line(colour = "#d0d0d0")) + 
  xlab ('subtree sizes') + ylab('cdf')
...
plotdepths <- ggplot(df.depths_all, aes(x=depth, y=cumprob, color=data)) + 
  geom_point(aes(x=depth, y = cumprob), shape = 21, size = 1, stroke = 1) +
  geom_line(aes(x=depth, y = cumprob), size=0.25) +
...
```{r}
params.Aragon2017 <- estimation_Aragon2017(data = df.trees %>% filter(train), params=list(alpha=0.3,
                                                                                          beta=2.5,
                                                                                          tau=0.8,
                                                                                          kappa=5))
params.Aragon2017
...
plotdegrees <- ggplot(df.degrees_all, aes(x=degree, y=cumprob, color=data)) + 
  ...
  geom_line(aes(x=degree, y = cumprob), size=0.25) +
  scale_x_log10(breaks=c(1,10,100)) +
  scale_y_log10(breaks=seq(0.1,1,0.1)) +
  theme_bw() + 
  theme(panel.grid.major = element_line(colour = "#d0d0d0")) + 
  ylab('cdf')
